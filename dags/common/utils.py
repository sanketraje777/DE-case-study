...
def insert_in_batches_execute_values(db_hook, table_name, df, batch_size=None):
    cols = df.columns.tolist()
    ...
    try:
        with conn.cursor() as cursor:
            if batch_size:
                ...
                execute_values(cursor, sql, data_gen)
        conn.commit()
...
def insert_all_execute_values(db_hook, table_name, df, *args, **kwargs):
    insert_in_batches_execute_values(db_hook, table_name, df)
...
def zip_lists(attributes, values):
    if not isinstance(attributes, (list, tuple)):
        attributes = [attributes]
    if not isinstance(values, (list, tuple)):
        values = [values]
    if len(attributes) != len(values):
        raise ValueError(f"Length mismatch: {len(attributes)} attributes vs {len(values)} values")
    return {k:v for k,v in zip(attributes, values)}

get_kwargs = zip_lists

def evaluate_sanity(*args):
    sanities = args[0]
    tables = args[1]
    tbl_idx_map = args[2]
    for tbl in tables:
        if tbl in tbl_idx_map:
            if not sanities[tbl_idx_map[tbl]][0]:
                return False
    return True

def is_sane(*values, success_val, failure_val, 
                 evaluation=lambda *values: all(values)):
