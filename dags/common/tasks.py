...
from common.utils import fetch_min_max_dt, fetch_min_max_num, fetch_count, insert_in_batches_copy_expert, fetch_in_chunks_cursor, convert_to_jinja
from common.chunking import generate_datetime_chunks, generate_num_chunks, generate_limit_offset_chunks
...
DAG_CONCURRENCY = int(os.environ.get("AIRFLOW__CORE__MAX_ACTIVE_RUNS_PER_DAG", 4))
...
def generate_datetime_chunks_task(
    ...
    """
    if query:
        # query = Template(convert_to_jinja(query)).render(dict(partial_params=partial_params, chunk_query_params=chunk_query_params, normal_params=normal_params))
...
def generate_limit_offset_task(
    ...
    """
    if query:
        # query = Template(convert_to_jinja(query)).render(dict(partial_params=partial_params, 
        #             chunk_query_params=chunk_query_params, normal_params=normal_params))
        # Render using Airflow's Jinja template helper
        query = Template(query).render(parameters or {})
...
def run_queries_with_task_params(hook_class, sql, parameters = None, handler:str = "run", hook_kwargs = {}):
    # if partial_params or chunk_query_params or normal_params:
    #     sql = [Template(convert_to_jinja(s)).render(dict(
    #         partial_params=partial_params, chunk_query_params=chunk_query_params, 
    #         normal_params=normal_params)) for s in sql]
...
@task()
def run_queries(hook_class, sql,
    parameters=None, handler: str="run", hook_kwargs={}, log_enabled=False):
    """
    Airflow task to:
    ...
    for single_sql in sql:
        ...
        # Render using Airflow's Jinja template helper
        single_sql = Template(single_sql).render(parameters or {})
