...
from common.tasks import DATETIME_INTERVAL, POSTGRES_CONN, CORE_PARALLELISM
...
with DAG(
    ...
    init_lz = EmptyOperator(task_id="init_lz")
    end_lz = EmptyOperator(task_id="end_lz")
    ...
    with TaskGroup(f"tg_{target_table}") as tg_items:
        ...
        chunk_items = generate_limit_offset_task.override(task_id=f"chunks_{target_table}")(
            query=f"""
                ...
                FROM (
                    SELECT DISTINCT si.* 
                    FROM {source_schema}.{source_table} si      -- ounass_source.salesorderitem
                    JOIN salesorder s 
                    ON s.id = si.order_id 
        ...
        process_items = process_chunk.override(
    ...
    with TaskGroup(f"tg_{target_table}") as tg_orders:
        ...
        chunk_orders = generate_limit_offset_task.override(task_id=f"chunks_{target_table}")(
            query=f"""
                ...
                FROM (
                    SELECT DISTINCT s.* 
                    FROM {source_schema}.{source_table} s       -- ounass_source.salesorder
        ...
        process_orders = process_chunk.override(
            task_id=f"process_{source_table}_chunks",
            pool=db_pool, max_active_tis_per_dag=CORE_PARALLELISM).partial(
            query=f"""
                SELECT DISTINCT s.* 
    ...
    with TaskGroup(f"tg_{target_table}") as tg_customers:
        chunk_customers = generate_limit_offset_task.override(task_id=f"chunks_{target_table}")(
            query=f"""
                ...
                FROM (
                    SELECT DISTINCT c.* 
                    FROM {source_schema}.{source_table} c       -- ounass_source.customer 
                    JOIN salesorder s 
        ...
        process_customers = process_chunk.override(
            task_id=f"process_{source_table}_chunks",
            pool=db_pool, max_active_tis_per_dag=CORE_PARALLELISM).partial(
            query=f"""
                SELECT DISTINCT c.* 
            ...
            sql=f"""
                CREATE INDEX IF NOT EXISTS 
                    idx_{target_table}_id 
                        ON {target_schema}.{target_table} (id);
                """
        )
    ...
    init_lz >> load_range >> [tg_items, tg_orders, tg_customers] >> \
        update_last_loaded >> end_lz >> trigger_etl_ods_dag
    init_lz >> variables >> [tg_items, tg_orders, tg_customers]
