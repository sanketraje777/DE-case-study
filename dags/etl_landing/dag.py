...
with DAG(
    ...
    for i, tgt in enumerate(target["tables"]):
        ...
        truncate_sql = sql_defs["truncate"].format(
            target_schema=target["schema"], target_table=tgt
        ).strip()
        
        deindex_sql = sql_defs["deindex"].format(
            target_schema=target["schema"], target_table=tgt
        ).strip()
            ...
            with TaskGroup(group_id=f"tg_preprocess_{tgt}") as tg_pre:
                ...
                chunk_sanity = check_sanity.override(task_id=f"{src_tbl}_chunk_sanity")(
                    ...
                    route_valid=[f"tg_{tgt}.tg_preprocess_{tgt}.prepare_{src_tbl}_parameters",
                                 f"tg_{tgt}.tg_preprocess_{tgt}.deindex_target_{tgt}"], 
                    route_invalid=f"tg_{tgt}.tg_postprocess_{tgt}.mark_last_loaded_{tgt}"
                )

                # Drop target indexes
                deindex_target = SQLExecuteQueryOperator(
                    task_id=f'deindex_target_{tgt}',
                    conn_id=POSTGRES_CONN["conn_id"],
                    sql=deindex_sql
                )
                ...
                chunk_source >> chunk_sanity >> [deindex_target, prepare_params]
...
for refs in tasks.values():
    refs["index"].trigger_rule = "all_success"
