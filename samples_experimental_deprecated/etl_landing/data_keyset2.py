...
from common.tasks import generate_datetime_chunks_task, generate_num_chunks_task
...
sql = {
    "salesorderitem": {
        ...
        """,
        "chunk": {
            "handler": generate_datetime_chunks_task,
            "kws": {"no_of_chunks": max(DAG_CONCURRENCY // 4, 1)},       # direct kwargs expansion
            "kwd": "range_time"             # rendered kwargs expansion (list[str] | str)
        },
        "process": """
            SELECT DISTINCT si.*
            FROM {source_schema}.{source_table} si
                JOIN salesorder s
                ON s.id = si.order_id
            WHERE (s.modified_at >= %({source_table}_start_val)s
                    AND s.modified_at < %({source_table}_end_val)s)
                OR (si.modified_at >= %({source_table}_start_val)s
                    AND si.modified_at < %({source_table}_end_val)s)
        """,
        "index": """
    ...
    "salesorder": {
        ...
        "process": """
            ...
            FROM {source_schema}.{source_table} s
                ...
                ON si.order_id = s.id
            WHERE (si.modified_at >= %({source_table}_start_val)s
                    AND si.modified_at < %({source_table}_end_val)s)
                OR (s.modified_at >= %({source_table}_start_val)s
                    AND s.modified_at < %({source_table}_end_val)s)
        """,
        "index": """
