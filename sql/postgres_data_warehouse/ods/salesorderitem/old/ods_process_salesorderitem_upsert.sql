  ...
  WHILE inner_offset < outer_limit LOOP
    ...
    deduped_items AS (
      SELECT
        ...
        (ARRAY_AGG(price       ORDER BY modified_at DESC))[1]::NUMERIC(10,2) AS price,
        (ARRAY_AGG(line_total  ORDER BY modified_at DESC))[1]::NUMERIC(12,2) AS line_total,
    ...
    WHILE start_idx <= total_rows LOOP
      ...
      WHERE
        ({{target_schema}}.{{target_table}}.order_id,   -- ods.salesorderitem.order_id
        {{target_schema}}.{{target_table}}.product_id,
        {{target_schema}}.{{target_table}}.qty_ordered,
        {{target_schema}}.{{target_table}}.price,
        {{target_schema}}.{{target_table}}.line_total,
