...
BEGIN
  ...
  WITH cleaned_raw AS NOT MATERIALIZED (
    SELECT
      ...
      ri.modified_at
    FROM {{source_schema}}.{{source_table[0]}} AS ri        -- ods.salesorderitem
    JOIN {{source_schema}}.{{source_table[1]}} AS so        -- ods.salesorder
      ON ri.order_id = so.id
    JOIN {{source_schema}}.{{source_table[2]}} AS cust      -- ods.customer
      ON so.customer_id = cust.id
    JOIN {{source_schema}}.{{source_table[3]}} AS prod      -- ods.product
