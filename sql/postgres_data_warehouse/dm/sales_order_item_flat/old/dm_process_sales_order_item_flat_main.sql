  ...
  WHILE inner_offset < outer_limit LOOP
    ...
    joined_page AS (
      SELECT
        ri.item_id,
        ri.order_id,
        TRIM(so.order_number),
        so.created_at   AS order_created_at,
        so.order_total,
        so.total_qty_ordered,
        cust.id         AS customer_id,
        TRIM(cust.first_name || ' ' || cust.last_name) AS customer_name,
        TRIM(cust.gender)     AS customer_gender,
        TRIM(cust.email)      AS customer_email,
        prod.product_id,
        TRIM(prod.product_sku),
        TRIM(prod.product_name),
      ...
      WHERE ri.item_id      IS NOT NULL
        ...
        AND (cust.gender IS NULL OR TRIM(cust.gender) IN ('Female','Male'))
