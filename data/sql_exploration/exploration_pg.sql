...
select * from public.load_audit;
select MAX(item_id) from lz.salesorderitem;
select * from ods.salesorderitem;
select * from ods.product;
select * from lz.salesorder;
select * from ods.salesorder;
...
select item_id, COUNT(order_id) from lz.salesorderitem
group by item_id 
order by item_id
limit 100 offset 1000;

select item_id, COUNT(*) from lz.salesorderitem
group by item_id
having COUNT(*) > 1;

-- Check 1: Row counts
SELECT 'salesorderitem' AS table_name, COUNT(*) AS row_count FROM lz.salesorderitem;
select MAX(item_id), MAX(order_id), MAX(product_id) from lz.salesorderitem;
select MIN(item_id), MIN(order_id), MIN(product_id) from lz.salesorderitem;

CREATE TABLE public.salesorderitem_10000 AS
select * from lz.salesorderitem;

CREATE TABLE public.salesorder_10000 AS 
select * from lz.salesorder;

CREATE TABLE public.customer_10000 AS
select * from lz.customer;

(select * from public.salesorderitem_2000)
EXCEPT ALL
(select * from public.salesorderitem_10000);

(select * from public.salesorderitem_10000)
EXCEPT ALL
(select * from public.salesorderitem_2000);

(select * from public.salesorder_2000)
EXCEPT ALL
(select * from public.salesorder_10000);

(select * from public.salesorder_10000)
EXCEPT ALL
(select * from public.salesorder_2000);

(select * from public.customer_2000)
EXCEPT ALL
(select * from public.customer_10000);

(select * from public.customer_10000)
EXCEPT ALL
(select * from public.customer_2000);

select * from lz.salesorderitem where item_id = 1414;
select * from ods.product where product_id = 62821;

-- Check 2: Critical columns NOT NULL
SELECT COUNT(*) AS salesorderitem_qty_ordered_negative FROM lz.salesorderitem WHERE qty_ordered < 0;
SELECT COUNT(*) AS salesorderitem_price_negative FROM lz.salesorderitem WHERE price < 0;
SELECT COUNT(*) AS salesorderitem_line_total_negative FROM lz.salesorderitem WHERE line_total < 0;
SELECT COUNT(*) AS salesorderitem_product_sku_nulls FROM lz.salesorderitem WHERE product_sku IS NULL;
SELECT COUNT(*) AS salesorderitem_product_name_nulls FROM lz.salesorderitem WHERE product_name IS NULL;

-- Check 3: Foreign key integrity
SELECT COUNT(*) AS orphaned_salesorderitem_orders FROM lz.salesorderitem WHERE order_id NOT IN (SELECT id FROM lz.salesorder);
SELECT COUNT(*) AS orphaned_salesorderitem_products FROM lz.salesorderitem WHERE product_id NOT IN (SELECT product_id FROM lz.salesorderitem);

-- Check 4: Unique constraints
SELECT product_sku, COUNT(*) FROM lz.salesorderitem GROUP BY product_sku HAVING COUNT(*) > 1;

SELECT * FROM analysis.monthly_sales_summary LIMIT 10;
